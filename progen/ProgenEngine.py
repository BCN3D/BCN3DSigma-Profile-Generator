...
def cura2Profile():
            ...
            for filament in sorted(PS.profilesData['filament'], key=lambda k: k['id']):
                for quality in sorted(PS.profilesData['quality'], key=lambda k: k['index']):
                    ...
                    if layerHeight not in globalQualities:
                        ...
                        qualityFile.append('quality_type = laye'+("%.2f" % layerHeight)+'mm')
                    ...
                    # resolution
                    qualityFile.append('layer_height = '+("%.2f" % layerHeight))
                    qualityFile.append('line_width = =machine_nozzle_size * 0.875')
    ...
    postProcessing.append('                        break')
    postProcessing.append(r'                layer = "\n".join(lines)')
    postProcessing.append('                data[index] = laye')
    ...
    postProcessing.append('                    try:')
    postProcessing.append('                        line = lines[tempIndex]')
    postProcessing.append('                        # Get retract value before starting the first laye')
    ...
    postProcessing.append('                                        if " E" in line and "G92" not in line:')
    postProcessing.append('                                            if eValue < self.getValue(line, "E"):')
    postProcessing.append('                                                purgeLength = round(self.getValue(line, "E") - eValue, 2)')
    ...
    postProcessing.append(r'                                                        lines[tempIndex] = lines[tempIndex]+" ; awesome\nT"+str(abs(countingForTool-1))+"\nT"+str(countingForTool)+"\nG91\nG1 F12000 Z2\nG90\nG1 F2400 E"+str(eValue+purgeLength)+"\nG1 F100 E"+str(eValue+2*purgeLength)+"\nG4 P2000\nG1 F2400 E"+str(eValue+purgeLength)+"\nG92 E"+str(eValue)+"\n"+lines[tempIndex+1]+"\nG91\nG1 F12000 Z-2\nG90 ; awesome"')
    postProcessing.append('                                                        del lines[tempIndex+1]')
    postProcessing.append('                                                        tempIndex -= 1')
    ...
    fileName = 'Cura 2/plugins/'+PS.machineSettingsPluginName+'/'+PS.machineSettingsPluginName+'.py'
    sigmaSettings = []
    sigmaSettings.append('from PyQt5.QtCore import pyqtProperty, pyqtSignal')
    sigmaSettings.append('from UM.FlameProfiler import pyqtSlot')
    sigmaSettings.append('')
    sigmaSettings.append('from cura.MachineAction import MachineAction')
    sigmaSettings.append('')
    sigmaSettings.append('from UM.Application import Application')
    sigmaSettings.append('from UM.Settings.InstanceContainer import InstanceContainer')
    sigmaSettings.append('from UM.Settings.ContainerRegistry import ContainerRegistry')
    sigmaSettings.append('from UM.Settings.DefinitionContainer import DefinitionContainer')
    sigmaSettings.append('from UM.Logger import Logger')
    sigmaSettings.append('')
    sigmaSettings.append('from cura.Settings.CuraContainerRegistry import CuraContainerRegistry')
    sigmaSettings.append('')
    sigmaSettings.append('import UM.i18n')
    sigmaSettings.append('catalog = UM.i18n.i18nCatalog("cura")')
    sigmaSettings.append('')
    sigmaSettings.append('')
    sigmaSettings.append('##  This action allows for certain settings that are "machine only") to be modified.')
    sigmaSettings.append('#   It automatically detects machine definitions that it knows how to change and attaches itself to those.')
    sigmaSettings.append('class '+PS.machineSettingsPluginName+'(MachineAction):')
    sigmaSettings.append('    def __init__(self, parent = None):')
    sigmaSettings.append('        super().__init__("'+PS.machineSettingsPluginName+'", catalog.i18nc("@action", "Sigma Settings"))')
    sigmaSettings.append('        self._qml_url = "'+PS.machineSettingsPluginName+'.qml"')
    sigmaSettings.append('')
    sigmaSettings.append('        self._container_index = 0')
    sigmaSettings.append('')
    sigmaSettings.append('        self._container_registry = ContainerRegistry.getInstance()')
    sigmaSettings.append('        self._container_registry.containerAdded.connect(self._onContainerAdded)')
    sigmaSettings.append('')
    sigmaSettings.append('    def _reset(self):')
    sigmaSettings.append('        global_container_stack = Application.getInstance().getGlobalContainerStack()')
    sigmaSettings.append('        if not global_container_stack:')
    sigmaSettings.append('            return')
    sigmaSettings.append('')
    sigmaSettings.append('        # Make sure there is a definition_changes container to store the machine settings')
    sigmaSettings.append('        definition_changes_container = global_container_stack.findContainer({"type": "definition_changes"})')
    sigmaSettings.append('        if not definition_changes_container:')
    sigmaSettings.append('            definition_changes_container = self._createDefinitionChangesContainer(global_container_stack)')
    sigmaSettings.append('')
    sigmaSettings.append('        # Notify the UI in which container to store the machine settings data')
    sigmaSettings.append('        container_index = global_container_stack.getContainerIndex(definition_changes_container)')
    sigmaSettings.append('        if container_index != self._container_index:')
    sigmaSettings.append('            self._container_index = container_index')
    sigmaSettings.append('            self.containerIndexChanged.emit()')
    sigmaSettings.append('')
    sigmaSettings.append('    def _createDefinitionChangesContainer(self, global_container_stack, container_index = None):')
    sigmaSettings.append('        definition_changes_container = InstanceContainer(global_container_stack.getName() + "_settings")')
    sigmaSettings.append('        definition = global_container_stack.getBottom()')
    sigmaSettings.append('        definition_changes_container.setDefinition(definition)')
    sigmaSettings.append('        definition_changes_container.addMetaDataEntry("type", "definition_changes")')
    sigmaSettings.append('')
    sigmaSettings.append('        self._container_registry.addContainer(definition_changes_container)')
    sigmaSettings.append('        # Insert definition_changes between the definition and the variant')
    sigmaSettings.append('        global_container_stack.insertContainer(-1, definition_changes_container)')
    sigmaSettings.append('')
    sigmaSettings.append('        return definition_changes_container')
    sigmaSettings.append('')
    sigmaSettings.append('    containerIndexChanged = pyqtSignal()')
    sigmaSettings.append('')
    sigmaSettings.append('    @pyqtProperty(int, notify = containerIndexChanged)')
    sigmaSettings.append('    def containerIndex(self):')
    sigmaSettings.append('        return self._container_index')
    sigmaSettings.append('')
    sigmaSettings.append('    def _onContainerAdded(self, container):')
    sigmaSettings.append('        # Add this action as a supported action to all machine definitions')
    sigmaSettings.append('        if isinstance(container, DefinitionContainer) and container.getMetaDataEntry("type") == "machine":')
    sigmaSettings.append('            if container.getProperty("machine_extruder_count", "value") > 1:')
    sigmaSettings.append('                # Multiextruder printers are not currently supported')
    sigmaSettings.append('                Logger.log("d", "Not attaching '+PS.machineSettingsPluginName+' to %s; Multi-extrusion printers are not supported", container.getId())')
    sigmaSettings.append('                return')
    sigmaSettings.append('')
    sigmaSettings.append('            Application.getInstance().getMachineActionManager().addSupportedAction(container.getId(), self.getKey())')
    sigmaSettings.append('')
    sigmaSettings.append('    @pyqtSlot()')
    sigmaSettings.append('    def forceUpdate(self):')
    sigmaSettings.append('        # Force rebuilding the build volume by reloading the global container stack.')
    sigmaSettings.append('        # This is a bit of a hack, but it seems quick enough.')
    sigmaSettings.append('        Application.getInstance().globalContainerStackChanged.emit()')
    sigmaSettings.append('')
    sigmaSettings.append('    @pyqtSlot()')
    sigmaSettings.append('    def updateHasMaterialsMetadata(self):')
    sigmaSettings.append('        # Updates the has_materials metadata flag after switching gcode flavor')
    sigmaSettings.append('        global_container_stack = Application.getInstance().getGlobalContainerStack()')
    sigmaSettings.append('        if global_container_stack:')
    sigmaSettings.append('            definition = global_container_stack.getBottom()')
    sigmaSettings.append('            if definition.getProperty("machine_gcode_flavor", "value") == "UltiGCode" and not definition.getMetaDataEntry("has_materials", False):')
    sigmaSettings.append('                has_materials = global_container_stack.getProperty("machine_gcode_flavor", "value") != "UltiGCode"')
    sigmaSettings.append('')
    sigmaSettings.append('                material_container = global_container_stack.findContainer({"type": "material"})')
    sigmaSettings.append('                material_index = global_container_stack.getContainerIndex(material_container)')
    sigmaSettings.append('')
    sigmaSettings.append('                if has_materials:')
    sigmaSettings.append('                    if "has_materials" in global_container_stack.getMetaData():')
    sigmaSettings.append('                        global_container_stack.setMetaDataEntry("has_materials", True)')
    sigmaSettings.append('                    else:')
    sigmaSettings.append('                        global_container_stack.addMetaDataEntry("has_materials", True)')
    sigmaSettings.append('')
    sigmaSettings.append('                    # Set the material container to a sane default')
    sigmaSettings.append('                    if material_container.getId() == "empty_material":')
    sigmaSettings.append('                        search_criteria = { "type": "material", "definition": "fdmprinter", "id": "*pla*" }')
    sigmaSettings.append('                        containers = self._container_registry.findInstanceContainers(**search_criteria)')
    sigmaSettings.append('                        if containers:')
    sigmaSettings.append('                            global_container_stack.replaceContainer(material_index, containers[0])')
    sigmaSettings.append('                else:')
    sigmaSettings.append('                    # The metadata entry is stored in an ini, and ini files are parsed as strings only.')
    sigmaSettings.append('                    # Because any non-empty string evaluates to a boolean True, we have to remove the entry to make it False.')
    sigmaSettings.append('                    if "has_materials" in global_container_stack.getMetaData():')
    sigmaSettings.append('                        global_container_stack.removeMetaDataEntry("has_materials")')
    sigmaSettings.append('')
    sigmaSettings.append('                    empty_material = self._container_registry.findInstanceContainers(id = "empty_material")[0]')
    sigmaSettings.append('                    global_container_stack.replaceContainer(material_index, empty_material)')
    sigmaSettings.append('')
    sigmaSettings.append('                Application.getInstance().globalContainerStackChanged.emit()')
    fileContent = '\n'.join(sigmaSettings)
    filesList.append((fileName, fileContent))
