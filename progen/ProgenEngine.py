...
def simplify3DProfile(hotendLeft, hotendRight, filamentLeft, filamentRight):
    ...
    for extruder in extruderPrintOptions:
        for quality in sorted(PS.profilesData['quality'], key=lambda k: k['index']):
            ...
            fff.append('    <postProcessing>{REPLACE "; outer perimeter" "; outer perimeterM204 S'+str(reducedAccelerationForPerimeters)+'"},{REPLACE "; inner perimeter" "; inner perimeterM204 S2000"},{REPLACE "; solid layer" "; solid layerM204 S2000"},{REPLACE "; infill" "; infillM204 S2000",{REPLACE "; support" "; supportM204 S2000"},{REPLACE "; layer end" "; layer endM204 S2000"},{REPLACE "F12000G1 Z'+str(round(layerHeight*firstLayerHeightPercentage/100., 3))+' F1002G92 E0" "F12000G1 Z'+str(round(layerHeight*firstLayerHeightPercentage/100., 3))+' F1002G1 E0.0000 F720G92 E0"}</postProcessing>')
...
def cura2Profile():
    ...
    for filament in sorted(PS.profilesData['filament'], key=lambda k: k['id']):
        ...
        material.append('            <color>'+filament['color']+'</color>')
        material.append('        </name>')
        material.append('        <GUID>'+str(uuid.uuid1())+'</GUID>')
        material.append('        <version>1</version>')
        material.append('        <color_code>'+filament['colorCode']+'</color_code>')
        ...
        if hotend['id'] != 'None':
            for filament in sorted(PS.profilesData['filament'], key=lambda k: k['id']):
                for quality in sorted(PS.profilesData['quality'], key=lambda k: k['index']):
                    ...
                    fileName = 'Cura 2/resources/quality/'+PS.cura2id+'/'+PS.cura2id+'_'+hotend['id'].replace(' ', '_')+'_'+filament['brand'].replace(' ', '_')+'_'+filament['material'].replace(' ', '_')+'_'+quality['id'].replace(' ', '_')+'_Quality.inst.cfg'
                    ...
                    # qualityFile.append('material_extrusion_cool_down_speed = 0.7') # this value depends on extruded flow (not material_flow)
                    qualityFile.append('material_bed_temperature = '+("%.2f" % filament['bedTemperature']))
                    ...
                    qualityFile.append('material_standby_temperature = '+("%.2f" % standbyTemperature))
    ...
    postProcessing.append('                    "description": "Avoid zeroing extruders at the beginning.",')
    postProcessing.append('                    "type": "bool",')
    postProcessing.append('                    "default_value": true')
    postProcessing.append('                },')
    ...
    postProcessing.append('                    "default_value": true')
    postProcessing.append('                },')
    postProcessing.append('                "minimumExtrusion":')
    postProcessing.append('                {')
    postProcessing.append('                    "label": "Minimum Extrusion",')
    ...
    postProcessing.append('                },')
    postProcessing.append('                "purgeLength":')
    postProcessing.append('                {')
    postProcessing.append('                    "label": "Purge Length",')
    ...
    postProcessing.append('                    "default_value": 25,')
    postProcessing.append('                    "minimum_value": "0",')
    postProcessing.append('                    "minimum_value_warning": "0",')
    ...
    postProcessing.append('        minimumExtrusion = self.getSettingValueByKey("minimumExtrusion")')
    postProcessing.append('        purgeLength = self.getSettingValueByKey("purgeLength")')
    ...
    postProcessing.append('')
    postProcessing.append('        if activeExtruders or fixTemperatureOscilation:')
    ...
    postProcessing.append('                        elif "G" in line and "X" in line and "Y" in line and "E" in line:')
    ...
    postProcessing.append('                                    if "G0" in line and "F" in line and "X" in line and "Y" in line and "Z" in line:')
    ...
    postProcessing.append('                                line = lines[tempIndex+lineCount]')
    postProcessing.append('                                if "G" in line and "F" in line and "E-" in line:')
    ...
    postProcessing.append('                            line4 = lines[tempIndex + 4]')
    postProcessing.append('                            line5 = lines[tempIndex + 5]')
    postProcessing.append('                            # Remove unintentional retract before T1')
    postProcessing.append('                            if tempIndex == 0 and "G1 F" in line1 and "E" in line1 and line2 =="G92 E0" and line4 == "T1" and line5 == "G92 E0":')
    postProcessing.append('                                del lines[tempIndex + 1]')
    postProcessing.append('                                del lines[tempIndex + 1]')
    postProcessing.append('                                tempIndex -= 1')
    postProcessing.append('                            # Add proper prime command to T1')
    postProcessing.append('                            elif line == "T1" and line1 == "G92 E0" and line2 == "G91" and "G1 F" in line3 and line4 == "G90":')
    postProcessing.append('                                lineCount = 6 # According to extruder_start_gcode in Sigma Extruders definitions')
    postProcessing.append('                                while not lines[tempIndex+lineCount].startswith(";TYPE"):')
    postProcessing.append('                                    line = lines[tempIndex+lineCount]')
    postProcessing.append('                                    if "G0" in line and "F" in line and "X" in line and "Y" in line:')
    ...
    postProcessing.append('                    tempIndex = 0')
    postProcessing.append('                    while tempIndex < len(lines):')
    postProcessing.append('                        if layer.startswith(";LAYER:") and (lines[tempIndex].startswith("T0") or lines[tempIndex].startswith("T1")):')
    ...
    postProcessing.append('                                if "G" in line and "F" in line and "E" in line and self.getValue(line, "E") < minimumExtrusion:')
    ...
    postProcessing.append('                        correctTemperatureValue = max(self.getValue(temperatures[elementIndex][3], "S") - '+str(temperatureInertiaInitialFix)+', '+("%.2f" % standbyTemperature)+')')
    postProcessing.append('                        temperatures[elementIndex][3] = temperatures[elementIndex][3].split("S")[0]+"S"+str(correctTemperatureValue)')
    ...
    # fileName = 'Cura 2/plugins/'+PS.machineSettingsPluginName+'/__init__.py'
    # sigmaSettings = []
    # sigmaSettings.append('from . import '+PS.machineSettingsPluginName)
    # sigmaSettings.append('')
    # sigmaSettings.append('from UM.i18n import i18nCatalog')
    # sigmaSettings.append('catalog = i18nCatalog("cura")')
    # sigmaSettings.append('')
    # sigmaSettings.append('def getMetaData():')
    # sigmaSettings.append('    return {')
    # sigmaSettings.append('        "plugin": {')
    # sigmaSettings.append('            "name": catalog.i18nc("@label", "Sigma Settings action"),')
    # sigmaSettings.append('            "author": "BCN3DTechnologies",')
    # sigmaSettings.append('            "version": "1.0",')
    # sigmaSettings.append('            "description": catalog.i18nc("@info:whatsthis", "Provides a way to change Sigma settings"),')
    # sigmaSettings.append('            "api": 3')
    # sigmaSettings.append('        }')
    # sigmaSettings.append('    }')
    # sigmaSettings.append('')
    # sigmaSettings.append('def register(app):')
    # sigmaSettings.append('    return { "machine_action": '+PS.machineSettingsPluginName+'.'+PS.machineSettingsPluginName+'() }')
    # fileContent = '\n'.join(sigmaSettings)
    # filesList.append((fileName, fileContent))

    # for className in ['MachineSettingsAction', PS.machineSettingsPluginName]:
    #     fileName = 'Cura 2/plugins/'+className+'/'+className+'.py'
    #     machineSettings = []
    #     machineSettings.append('# Copyright (c) 2017 Ultimaker B.V.')
    #     machineSettings.append('# Cura is released under the terms of the AGPLv3 or higher.')
    #     machineSettings.append('')
    #     machineSettings.append('from PyQt5.QtCore import pyqtProperty, pyqtSignal')
    #     machineSettings.append('from UM.FlameProfiler import pyqtSlot')
    #     machineSettings.append('')
    #     machineSettings.append('from cura.MachineAction import MachineAction')
    #     machineSettings.append('')
    #     machineSettings.append('from UM.Application import Application')
    #     machineSettings.append('from UM.Preferences import Preferences')
    #     machineSettings.append('from UM.Settings.InstanceContainer import InstanceContainer')
    #     machineSettings.append('from UM.Settings.ContainerRegistry import ContainerRegistry')
    #     machineSettings.append('from UM.Settings.DefinitionContainer import DefinitionContainer')
    #     machineSettings.append('from UM.Scene.Iterator.DepthFirstIterator import DepthFirstIterator')
    #     machineSettings.append('from UM.Logger import Logger')
    #     machineSettings.append('')
    #     machineSettings.append('from cura.CuraApplication import CuraApplication')
    #     machineSettings.append('from cura.Settings.ExtruderManager import ExtruderManager')
    #     machineSettings.append('')
    #     machineSettings.append('import UM.i18n')
    #     machineSettings.append('catalog = UM.i18n.i18nCatalog("cura")')
    #     machineSettings.append('')
    #     machineSettings.append('')
    #     machineSettings.append('##  This action allows for certain settings that are "machine only") to be modified.')
    #     machineSettings.append('#   It automatically detects machine definitions that it knows how to change and attaches itself to those.')
    #     machineSettings.append('class '+className+'(MachineAction):')
    #     machineSettings.append('    def __init__(self, parent = None):')
    #     machineSettings.append('        super().__init__("'+className+'", catalog.i18nc("@action", "'+''.join(' ' + x if x.isupper() else x for x in className[:-6])[1:]+'"))')
    #     machineSettings.append('        self._qml_url = "'+className+'.qml"')            
    #     machineSettings.append('')
    #     machineSettings.append('        self._global_container_stack = None')
    #     machineSettings.append('        self._container_index = 0')
    #     machineSettings.append('        self._extruder_container_index = 0')
    #     machineSettings.append('')
    #     machineSettings.append('        self._container_registry = ContainerRegistry.getInstance()')
    #     machineSettings.append('        self._container_registry.containerAdded.connect(self._onContainerAdded)')
    #     machineSettings.append('        self._container_registry.containerRemoved.connect(self._onContainerRemoved)')
    #     machineSettings.append('        Application.getInstance().globalContainerStackChanged.connect(self._onGlobalContainerChanged)')
    #     machineSettings.append('        ExtruderManager.getInstance().activeExtruderChanged.connect(self._onActiveExtruderStackChanged)')
    #     machineSettings.append('')
    #     machineSettings.append('        self._backend = Application.getInstance().getBackend()')
    #     machineSettings.append('')
    #     machineSettings.append('    def _onContainerAdded(self, container):')
    #     machineSettings.append('        # Add this action as a supported action to all machine definitions')
    #     if PS.machineSettingsPluginName == className:
    #         machineSettings.append('        if isinstance(container, DefinitionContainer) and container.getMetaDataEntry("type") == "machine"  and "sigma" in container.getId():')
    #     else:
    #         machineSettings.append('        if isinstance(container, DefinitionContainer) and container.getMetaDataEntry("type") == "machine"  and "sigma" not in container.getId():')
    #     machineSettings.append('            Application.getInstance().getMachineActionManager().addSupportedAction(container.getId(), self.getKey())')
    #     machineSettings.append('')
    #     machineSettings.append('    def _onContainerRemoved(self, container):')
    #     machineSettings.append('        # Remove definition_changes containers when a stack is removed')
    #     machineSettings.append('        if container.getMetaDataEntry("type") in ["machine", "extruder_train"]:')
    #     machineSettings.append('            definition_changes_container = container.findContainer({"type": "definition_changes"})')
    #     machineSettings.append('            if not definition_changes_container:')
    #     machineSettings.append('                return')
    #     machineSettings.append('')
    #     machineSettings.append('            self._container_registry.removeContainer(definition_changes_container.getId())')
    #     machineSettings.append('')
    #     machineSettings.append('    def _reset(self):')
    #     machineSettings.append('        if not self._global_container_stack:')
    #     machineSettings.append('            return')
    #     machineSettings.append('')
    #     machineSettings.append('        # Make sure there is a definition_changes container to store the machine settings')
    #     machineSettings.append('        definition_changes_container = self._global_container_stack.findContainer({"type": "definition_changes"})')
    #     machineSettings.append('        if not definition_changes_container:')
    #     machineSettings.append('            definition_changes_container = self._createDefinitionChangesContainer(self._global_container_stack, self._global_container_stack.getName() + "_settings")')
    #     machineSettings.append('')
    #     machineSettings.append('        # Notify the UI in which container to store the machine settings data')
    #     machineSettings.append('        container_index = self._global_container_stack.getContainerIndex(definition_changes_container)')
    #     machineSettings.append('        if container_index != self._container_index:')
    #     machineSettings.append('            self._container_index = container_index')
    #     machineSettings.append('            self.containerIndexChanged.emit()')
    #     machineSettings.append('')
    #     machineSettings.append('        # Disable autoslicing while the machineaction is showing')
    #     machineSettings.append('        self._backend.disableTimer()')
    #     machineSettings.append('')
    #     machineSettings.append('    @pyqtSlot()')
    #     machineSettings.append('    def onFinishAction(self):')
    #     machineSettings.append('        # Restore autoslicing when the machineaction is dismissed')
    #     machineSettings.append('        if self._backend.determineAutoSlicing():')
    #     machineSettings.append('            self._backend.tickle()')
    #     machineSettings.append('')
    #     machineSettings.append('    def _onActiveExtruderStackChanged(self):')
    #     machineSettings.append('        extruder_container_stack = ExtruderManager.getInstance().getActiveExtruderStack()')
    #     machineSettings.append('        if not self._global_container_stack or not extruder_container_stack:')
    #     machineSettings.append('            return')
    #     machineSettings.append('')
    #     machineSettings.append('        # Make sure there is a definition_changes container to store the machine settings')
    #     machineSettings.append('        definition_changes_container = extruder_container_stack.findContainer({"type": "definition_changes"})')
    #     machineSettings.append('        if not definition_changes_container:')
    #     machineSettings.append('            definition_changes_container = self._createDefinitionChangesContainer(extruder_container_stack, extruder_container_stack.getId() + "_settings")')
    #     machineSettings.append('')
    #     machineSettings.append('        # Notify the UI in which container to store the machine settings data')
    #     machineSettings.append('        container_index = extruder_container_stack.getContainerIndex(definition_changes_container)')
    #     machineSettings.append('        if container_index != self._extruder_container_index:')
    #     machineSettings.append('            self._extruder_container_index = container_index')
    #     machineSettings.append('            self.extruderContainerIndexChanged.emit()')
    #     machineSettings.append('')
    #     machineSettings.append('    def _createDefinitionChangesContainer(self, container_stack, container_name, container_index = None):')
    #     machineSettings.append('        definition_changes_container = InstanceContainer(container_name)')
    #     machineSettings.append('        definition = container_stack.getBottom()')
    #     machineSettings.append('        definition_changes_container.setDefinition(definition)')
    #     machineSettings.append('        definition_changes_container.addMetaDataEntry("type", "definition_changes")')
    #     machineSettings.append('        definition_changes_container.addMetaDataEntry("setting_version", CuraApplication.SettingVersion)')
    #     machineSettings.append('')
    #     machineSettings.append('        self._container_registry.addContainer(definition_changes_container)')
    #     machineSettings.append('        container_stack.definitionChanges = definition_changes_container')
    #     machineSettings.append('')
    #     machineSettings.append('        return definition_changes_container')
    #     machineSettings.append('')
    #     machineSettings.append('    containerIndexChanged = pyqtSignal()')
    #     machineSettings.append('')
    #     machineSettings.append('    @pyqtProperty(int, notify = containerIndexChanged)')
    #     machineSettings.append('    def containerIndex(self):')
    #     machineSettings.append('        return self._container_index')
    #     machineSettings.append('')
    #     machineSettings.append('    extruderContainerIndexChanged = pyqtSignal()')
    #     machineSettings.append('')
    #     machineSettings.append('    @pyqtProperty(int, notify = extruderContainerIndexChanged)')
    #     machineSettings.append('    def extruderContainerIndex(self):')
    #     machineSettings.append('        return self._extruder_container_index')
    #     machineSettings.append('')
    #     machineSettings.append('')
    #     machineSettings.append('    def _onGlobalContainerChanged(self):')
    #     machineSettings.append('        self._global_container_stack = Application.getInstance().getGlobalContainerStack()')
    #     machineSettings.append('')
    #     machineSettings.append('        # This additional emit is needed because we cannot connect a UM.Signal directly to a pyqtSignal')
    #     machineSettings.append('        self.globalContainerChanged.emit()')
    #     machineSettings.append('')
    #     machineSettings.append('    globalContainerChanged = pyqtSignal()')
    #     machineSettings.append('')
    #     machineSettings.append('    @pyqtProperty(int, notify = globalContainerChanged)')
    #     machineSettings.append('    def definedExtruderCount(self):')
    #     machineSettings.append('        if not self._global_container_stack:')
    #     machineSettings.append('            return 0')
    #     machineSettings.append('')
    #     machineSettings.append('        return len(self._global_container_stack.getMetaDataEntry("machine_extruder_trains"))')
    #     machineSettings.append('')
    #     machineSettings.append('    @pyqtSlot(int)')
    #     machineSettings.append('    def setMachineExtruderCount(self, extruder_count):')
    #     machineSettings.append('        machine_manager = Application.getInstance().getMachineManager()')
    #     machineSettings.append('        extruder_manager = ExtruderManager.getInstance()')
    #     machineSettings.append('')
    #     machineSettings.append('        definition_changes_container = self._global_container_stack.findContainer({"type": "definition_changes"})')
    #     machineSettings.append('        if not self._global_container_stack or not definition_changes_container:')
    #     machineSettings.append('            return')
    #     machineSettings.append('')
    #     machineSettings.append('        previous_extruder_count = self._global_container_stack.getProperty("machine_extruder_count", "value")')
    #     machineSettings.append('        if extruder_count == previous_extruder_count:')
    #     machineSettings.append('            return')
    #     machineSettings.append('')
    #     machineSettings.append('        extruder_material_id = None')
    #     machineSettings.append('        extruder_variant_id = None')
    #     machineSettings.append('        if extruder_count == 1:')
    #     machineSettings.append('            # Get the material and variant of the first extruder before setting the number extruders to 1')
    #     machineSettings.append('            if machine_manager.hasMaterials:')
    #     machineSettings.append('                extruder_material_id = machine_manager.allActiveMaterialIds[extruder_manager.extruderIds["0"]]')
    #     machineSettings.append('            if machine_manager.hasVariants:')
    #     machineSettings.append('                extruder_variant_id = machine_manager.allActiveVariantIds[extruder_manager.extruderIds["0"]]')
    #     machineSettings.append('')
    #     machineSettings.append('            # Copy any settable_per_extruder setting value from the extruders to the global stack')
    #     machineSettings.append('            extruder_stacks = ExtruderManager.getInstance().getActiveExtruderStacks()')
    #     machineSettings.append('            extruder_stacks.reverse() # make sure the first extruder is done last, so its settings override any higher extruder settings')
    #     machineSettings.append('')
    #     machineSettings.append('            global_user_container = self._global_container_stack.getTop()')
    #     machineSettings.append('            for extruder_stack in extruder_stacks:')
    #     machineSettings.append('                extruder_index = extruder_stack.getMetaDataEntry("position")')
    #     machineSettings.append('                extruder_user_container = extruder_stack.getTop()')
    #     machineSettings.append('                for setting_instance in extruder_user_container.findInstances():')
    #     machineSettings.append('                    setting_key = setting_instance.definition.key')
    #     machineSettings.append('                    settable_per_extruder = self._global_container_stack.getProperty(setting_key, "settable_per_extruder")')
    #     machineSettings.append('                    if settable_per_extruder:')
    #     machineSettings.append('                        limit_to_extruder = self._global_container_stack.getProperty(setting_key, "limit_to_extruder")')
    #     machineSettings.append('')
    #     machineSettings.append('                        if limit_to_extruder == "-1" or limit_to_extruder == extruder_index:')
    #     machineSettings.append('                            global_user_container.setProperty(setting_key, "value", extruder_user_container.getProperty(setting_key, "value"))')
    #     machineSettings.append('                            extruder_user_container.removeInstance(setting_key)')
    #     machineSettings.append('')
    #     machineSettings.append('        # Check to see if any features are set to print with an extruder that will no longer exist')
    #     machineSettings.append('        for setting_key in ["adhesion_extruder_nr", "support_extruder_nr", "support_extruder_nr_layer_0", "support_infill_extruder_nr", "support_interface_extruder_nr"]:')
    #     machineSettings.append('            if int(self._global_container_stack.getProperty(setting_key, "value")) > extruder_count - 1:')
    #     machineSettings.append(r'                Logger.log("i", "Lowering %s setting to match number of extruders", setting_key)')
    #     machineSettings.append('                self._global_container_stack.getTop().setProperty(setting_key, "value", extruder_count - 1)')
    #     machineSettings.append('')
    #     machineSettings.append('        # Check to see if any objects are set to print with an extruder that will no longer exist')
    #     machineSettings.append('        root_node = Application.getInstance().getController().getScene().getRoot()')
    #     machineSettings.append('        for node in DepthFirstIterator(root_node):')
    #     machineSettings.append('            if node.getMeshData():')
    #     machineSettings.append('                extruder_nr = node.callDecoration("getActiveExtruderPosition")')
    #     machineSettings.append('')
    #     machineSettings.append('                if extruder_nr is not None and int(extruder_nr) > extruder_count - 1:')
    #     machineSettings.append('                    node.callDecoration("setActiveExtruder", extruder_manager.getExtruderStack(extruder_count - 1).getId())')
    #     machineSettings.append('')
    #     machineSettings.append('        definition_changes_container.setProperty("machine_extruder_count", "value", extruder_count)')
    #     machineSettings.append('        self.forceUpdate()')
    #     machineSettings.append('')
    #     machineSettings.append('        if extruder_count > 1:')
    #     machineSettings.append('            # Multiextrusion')
    #     machineSettings.append('')
    #     machineSettings.append('            # Make sure one of the extruder stacks is active')
    #     machineSettings.append('            if extruder_manager.activeExtruderIndex == -1:')
    #     machineSettings.append('                extruder_manager.setActiveExtruderIndex(0)')
    #     machineSettings.append('')
    #     machineSettings.append('            # Move settable_per_extruder values out of the global container')
    #     machineSettings.append('            if previous_extruder_count == 1:')
    #     machineSettings.append('                extruder_stacks = ExtruderManager.getInstance().getActiveExtruderStacks()')
    #     machineSettings.append('                global_user_container = self._global_container_stack.getTop()')
    #     machineSettings.append('')
    #     machineSettings.append('                for setting_instance in global_user_container.findInstances():')
    #     machineSettings.append('                    setting_key = setting_instance.definition.key')
    #     machineSettings.append('                    settable_per_extruder = self._global_container_stack.getProperty(setting_key, "settable_per_extruder")')
    #     machineSettings.append('                    if settable_per_extruder:')
    #     machineSettings.append('                        limit_to_extruder = int(self._global_container_stack.getProperty(setting_key, "limit_to_extruder"))')
    #     machineSettings.append('                        extruder_stack = extruder_stacks[max(0, limit_to_extruder)]')
    #     machineSettings.append('                        extruder_stack.getTop().setProperty(setting_key, "value", global_user_container.getProperty(setting_key, "value"))')
    #     machineSettings.append('                        global_user_container.removeInstance(setting_key)')
    #     machineSettings.append('        else:')
    #     machineSettings.append('            # Single extrusion')
    #     machineSettings.append('')
    #     machineSettings.append('            # Make sure the machine stack is active')
    #     machineSettings.append('            if extruder_manager.activeExtruderIndex > -1:')
    #     machineSettings.append('                extruder_manager.setActiveExtruderIndex(-1)')
    #     machineSettings.append('')
    #     machineSettings.append('            # Restore material and variant on global stack')
    #     machineSettings.append('            # MachineManager._onGlobalContainerChanged removes the global material and variant of multiextruder machines')
    #     machineSettings.append('            if extruder_material_id or extruder_variant_id:')
    #     machineSettings.append('                # Prevent the DiscardOrKeepProfileChangesDialog from popping up (twice) if there are user changes')
    #     machineSettings.append('                # The dialog is not relevant here, since we'+"'re restoring the previous situation as good as possible")
    #     machineSettings.append('                preferences = Preferences.getInstance()')
    #     machineSettings.append('                choice_on_profile_override = preferences.getValue("cura/choice_on_profile_override")')
    #     machineSettings.append('                preferences.setValue("cura/choice_on_profile_override", "always_keep")')
    #     machineSettings.append('')
    #     machineSettings.append('                if extruder_material_id:')
    #     machineSettings.append('                    machine_manager.setActiveMaterial(extruder_material_id)')
    #     machineSettings.append('                if extruder_variant_id:')
    #     machineSettings.append('                    machine_manager.setActiveVariant(extruder_variant_id)')
    #     machineSettings.append('')
    #     machineSettings.append('                preferences.setValue("cura/choice_on_profile_override", choice_on_profile_override)')
    #     machineSettings.append('')
    #     machineSettings.append('')
    #     machineSettings.append('    @pyqtSlot()')
    #     machineSettings.append('    def forceUpdate(self):')
    #     machineSettings.append('        # Force rebuilding the build volume by reloading the global container stack.')
    #     machineSettings.append('        # This is a bit of a hack, but it seems quick enough.')
    #     machineSettings.append('        Application.getInstance().globalContainerStackChanged.emit()')
    #     machineSettings.append('')
    #     machineSettings.append('    @pyqtSlot()')
    #     machineSettings.append('    def updateHasMaterialsMetadata(self):')
    #     machineSettings.append('        # Updates the has_materials metadata flag after switching gcode flavor')
    #     machineSettings.append('        if not self._global_container_stack:')
    #     machineSettings.append('            return')
    #     machineSettings.append('')
    #     machineSettings.append('        definition = self._global_container_stack.getBottom()')
    #     machineSettings.append('        if definition.getProperty("machine_gcode_flavor", "value") == "UltiGCode" and not definition.getMetaDataEntry("has_materials", False):')
    #     machineSettings.append('            has_materials = self._global_container_stack.getProperty("machine_gcode_flavor", "value") != "UltiGCode"')
    #     machineSettings.append('')
    #     machineSettings.append('            material_container = self._global_container_stack.material')
    #     machineSettings.append('            material_index = self._global_container_stack.getContainerIndex(material_container)')
    #     machineSettings.append('')
    #     machineSettings.append('            if has_materials:')
    #     machineSettings.append('                if "has_materials" in self._global_container_stack.getMetaData():')
    #     machineSettings.append('                    self._global_container_stack.setMetaDataEntry("has_materials", True)')
    #     machineSettings.append('                else:')
    #     machineSettings.append('                    self._global_container_stack.addMetaDataEntry("has_materials", True)')
    #     machineSettings.append('')
    #     machineSettings.append('                # Set the material container to a sane default')
    #     machineSettings.append('                if material_container.getId() == "empty_material":')
    #     machineSettings.append('                    search_criteria = { "type": "material", "definition": "fdmprinter", "id": "*pla*"}')
    #     machineSettings.append('                    containers = self._container_registry.findInstanceContainers(**search_criteria)')
    #     machineSettings.append('                    if containers:')
    #     machineSettings.append('                        self._global_container_stack.replaceContainer(material_index, containers[0])')
    #     machineSettings.append('            else:')
    #     machineSettings.append('                # The metadata entry is stored in an ini, and ini files are parsed as strings only.')
    #     machineSettings.append('                # Because any non-empty string evaluates to a boolean True, we have to remove the entry to make it False.')
    #     machineSettings.append('                if "has_materials" in self._global_container_stack.getMetaData():')
    #     machineSettings.append('                    self._global_container_stack.removeMetaDataEntry("has_materials")')
    #     machineSettings.append('')
    #     machineSettings.append('                self._global_container_stack.material = ContainerRegistry.getInstance().getEmptyInstanceContainer()')
    #     machineSettings.append('')
    #     machineSettings.append('            Application.getInstance().globalContainerStackChanged.emit()')
    #     machineSettings.append('')
    #     fileContent = '\n'.join(machineSettings)
    #     filesList.append((fileName, fileContent))

    # fileName = 'Cura 2/plugins/'+PS.machineSettingsPluginName+'/'+PS.machineSettingsPluginName+'.qml'
    # sigmaSettings = []
    # sigmaSettings.append('import QtQuick 2.2')
    # sigmaSettings.append('import QtQuick.Controls 1.1')
    # sigmaSettings.append('import QtQuick.Layouts 1.1')
    # sigmaSettings.append('import QtQuick.Window 2.1')
    # sigmaSettings.append('')
    # sigmaSettings.append('import UM 1.2 as UM')
    # sigmaSettings.append('import Cura 1.0 as Cura')
    # sigmaSettings.append('')
    # sigmaSettings.append('')
    # sigmaSettings.append('Cura.MachineAction')
    # sigmaSettings.append('{')
    # sigmaSettings.append('    anchors.fill: parent;')
    # sigmaSettings.append('    Item')
    # sigmaSettings.append('    {')
    # sigmaSettings.append('        id: bedLevelMachineAction')
    # sigmaSettings.append('        anchors.fill: parent;')
    # sigmaSettings.append('')
    # sigmaSettings.append('        UM.I18nCatalog { id: catalog; name: "cura"; }')
    # sigmaSettings.append('')
    # sigmaSettings.append('        Label')
    # sigmaSettings.append('        {')
    # sigmaSettings.append('            id: pageTitle')
    # sigmaSettings.append('            width: parent.width')
    # sigmaSettings.append('            text: catalog.i18nc("@title", "Sigma Settings")')
    # sigmaSettings.append('            wrapMode: Text.WordWrap')
    # sigmaSettings.append('            font.pointSize: 18;')
    # sigmaSettings.append('        }')
    # sigmaSettings.append('        Label')
    # sigmaSettings.append('        {')
    # sigmaSettings.append('            id: pageDescription')
    # sigmaSettings.append('            anchors.top: pageTitle.bottom')
    # sigmaSettings.append('            anchors.topMargin: UM.Theme.getSize("default_margin").height')
    # sigmaSettings.append('            width: parent.width')
    # sigmaSettings.append('            wrapMode: Text.WordWrap')
    # sigmaSettings.append('            visible: "'+PS.cura2Name+'" in Cura.MachineManager.activeMachineId')
    # sigmaSettings.append('            text: catalog.i18nc("@label", "Attention! In order to get the best results with your BCN3D Sigma, please make sure the '+PS.cura2PostProcessingPluginName+' plugin is enabled under Extensions > Post Processing > Modify G-Code.")')
    # sigmaSettings.append('        }')
    # sigmaSettings.append('')
    # sigmaSettings.append('        Column')
    # sigmaSettings.append('        {')
    # sigmaSettings.append('            height: parent.height - y')
    # sigmaSettings.append('            width: parent.width - UM.Theme.getSize("default_margin").width')
    # sigmaSettings.append('            spacing: UM.Theme.getSize("default_margin").height')
    # sigmaSettings.append('')
    # sigmaSettings.append('            anchors.left: parent.left')
    # sigmaSettings.append('            anchors.top: pageDescription.bottom')
    # sigmaSettings.append('            anchors.topMargin: UM.Theme.getSize("default_margin").height')
    # sigmaSettings.append('')
    # sigmaSettings.append('            Row')
    # sigmaSettings.append('            {')
    # sigmaSettings.append('                width: parent.width')
    # sigmaSettings.append('                spacing: UM.Theme.getSize("default_margin").height')
    # sigmaSettings.append('')
    # sigmaSettings.append('                Column')
    # sigmaSettings.append('                {')
    # sigmaSettings.append('                    width: parent.width / 2')
    # sigmaSettings.append('                    spacing: UM.Theme.getSize("default_margin").height')
    # sigmaSettings.append('')
    # sigmaSettings.append('                    Label')
    # sigmaSettings.append('                    {')
    # sigmaSettings.append('                        text: catalog.i18nc("@label", "Printer Settings")')
    # sigmaSettings.append('                        font.bold: true')
    # sigmaSettings.append('                    }')
    # sigmaSettings.append('')
    # sigmaSettings.append('                    Grid')
    # sigmaSettings.append('                    {')
    # sigmaSettings.append('                        columns: 3')
    # sigmaSettings.append('                        columnSpacing: UM.Theme.getSize("default_margin").width')
    # sigmaSettings.append('')
    # sigmaSettings.append('                        Label')
    # sigmaSettings.append('                        {')
    # sigmaSettings.append('                            text: catalog.i18nc("@label", "X (Width)")')
    # sigmaSettings.append('                        }')
    # sigmaSettings.append('                        TextField')
    # sigmaSettings.append('                        {')
    # sigmaSettings.append('                            id: buildAreaWidthField')
    # sigmaSettings.append('                            text: machineWidthProvider.properties.value')
    # sigmaSettings.append('                            validator: RegExpValidator { regExp: /[0-9\.]{0,6}/ }')
    # sigmaSettings.append('                            onEditingFinished: { machineWidthProvider.setPropertyValue("value", text); manager.forceUpdate() }')
    # sigmaSettings.append('                        }')
    # sigmaSettings.append('                        Label')
    # sigmaSettings.append('                        {')
    # sigmaSettings.append('                            text: catalog.i18nc("@label", "mm")')
    # sigmaSettings.append('                        }')
    # sigmaSettings.append('')
    # sigmaSettings.append('                        Label')
    # sigmaSettings.append('                        {')
    # sigmaSettings.append('                            text: catalog.i18nc("@label", "Y (Depth)")')
    # sigmaSettings.append('                        }')
    # sigmaSettings.append('                        TextField')
    # sigmaSettings.append('                        {')
    # sigmaSettings.append('                            id: buildAreaDepthField')
    # sigmaSettings.append('                            text: machineDepthProvider.properties.value')
    # sigmaSettings.append('                            validator: RegExpValidator { regExp: /[0-9\.]{0,6}/ }')
    # sigmaSettings.append('                            onEditingFinished: { machineDepthProvider.setPropertyValue("value", text); manager.forceUpdate() }')
    # sigmaSettings.append('                        }')
    # sigmaSettings.append('                        Label')
    # sigmaSettings.append('                        {')
    # sigmaSettings.append('                            text: catalog.i18nc("@label", "mm")')
    ...
    # sigmaSettings.append('                        Label')
    # sigmaSettings.append('                        {')
    # sigmaSettings.append('                            text: catalog.i18nc("@label", "Z (Height)")')
    # sigmaSettings.append('                        }')
    # sigmaSettings.append('                        TextField')
    # sigmaSettings.append('                        {')
    # sigmaSettings.append('                            id: buildAreaHeightField')
    # sigmaSettings.append('                            text: machineHeightProvider.properties.value')
    # sigmaSettings.append('                            validator: RegExpValidator { regExp: /[0-9\.]{0,6}/ }')
    # sigmaSettings.append('                            onEditingFinished: { machineHeightProvider.setPropertyValue("value", text); manager.forceUpdate() }')
    # sigmaSettings.append('                        }')
    # sigmaSettings.append('                        Label')
    # sigmaSettings.append('                        {')
    # sigmaSettings.append('                            text: catalog.i18nc("@label", "mm")')
    ...
    # sigmaSettings.append('                    Column')
    # sigmaSettings.append('                    {')
    # sigmaSettings.append('                        CheckBox')
    # sigmaSettings.append('                        {')
    # sigmaSettings.append('                            id: printOneAtATimeCheckBox')
    # sigmaSettings.append('                            text: catalog.i18nc("@option:check", "Enable setting: Print Sequence")')
    # sigmaSettings.append( "                            checked: String(machinePrintOneAtATimeProvider.properties.enabled).toLowerCase() != 'false'")
    # sigmaSettings.append('                            onClicked: machinePrintOneAtATimeProvider.setPropertyValue("enabled", checked)')
    # sigmaSettings.append('                        }')
    # sigmaSettings.append('                        CheckBox')
    # sigmaSettings.append('                        {')
    # sigmaSettings.append('                            id: autoTemperatureCheckBox')
    # sigmaSettings.append('                            text: catalog.i18nc("@option:check", "Enable setting: Auto Temperature")')
    # sigmaSettings.append("                            checked: String(machineAutoTemperatureProvider.properties.enabled).toLowerCase() != 'false'")
    # sigmaSettings.append('                            onClicked: machineAutoTemperatureProvider.setPropertyValue("enabled", checked)')
    # sigmaSettings.append('                        }')
    # sigmaSettings.append('                    }')
    # sigmaSettings.append('                }')
    # sigmaSettings.append('')
    ...
    # sigmaSettings.append('                    Label')
    # sigmaSettings.append('                    {')
    # sigmaSettings.append('                        text: catalog.i18nc("@label", "Printhead Settings")')
    # sigmaSettings.append('                        font.bold: true')
    # sigmaSettings.append('                    }')
    # sigmaSettings.append('')
    # sigmaSettings.append('                    Grid')
    ...
    # sigmaSettings.append('                        Label')
    # sigmaSettings.append('                        {')
    # sigmaSettings.append('                            text: catalog.i18nc("@label", "X min")')
    # sigmaSettings.append('                        }')
    # sigmaSettings.append('                        TextField')
    # sigmaSettings.append('                        {')
    # sigmaSettings.append('                            id: printheadXMinField')
    # sigmaSettings.append('                            text: getHeadPolygonCoord("x", "min")')
    # sigmaSettings.append('                            validator: RegExpValidator { regExp: /[0-9\.]{0,6}/ }')
    # sigmaSettings.append('                            onEditingFinished: setHeadPolygon()')
    # sigmaSettings.append('                        }')
    # sigmaSettings.append('                        Label')
    # sigmaSettings.append('                        {')
    # sigmaSettings.append('                            text: catalog.i18nc("@label", "mm")')
    ...
    # sigmaSettings.append('                        Label')
    # sigmaSettings.append('                        {')
    # sigmaSettings.append('                            text: catalog.i18nc("@label", "Y min")')
    # sigmaSettings.append('                        }')
    # sigmaSettings.append('                        TextField')
    # sigmaSettings.append('                        {')
    # sigmaSettings.append('                            id: printheadYMinField')
    # sigmaSettings.append('                            text: getHeadPolygonCoord("y", "min")')
    # sigmaSettings.append('                            validator: RegExpValidator { regExp: /[0-9\.]{0,6}/ }')
    # sigmaSettings.append('                            onEditingFinished: setHeadPolygon()')
    # sigmaSettings.append('                        }')
    # sigmaSettings.append('                        Label')
    ...
    # sigmaSettings.append('                        Label')
    # sigmaSettings.append('                        {')
    # sigmaSettings.append('                            text: catalog.i18nc("@label", "X max")')
    # sigmaSettings.append('                        }')
    # sigmaSettings.append('                        TextField')
    # sigmaSettings.append('                        {')
    # sigmaSettings.append('                            id: printheadXMaxField')
    # sigmaSettings.append('                            text: getHeadPolygonCoord("x", "max")')
    # sigmaSettings.append('                            validator: RegExpValidator { regExp: /[0-9\.]{0,6}/ }')
    # sigmaSettings.append('                            onEditingFinished: setHeadPolygon()')
    # sigmaSettings.append('                        }')
    # sigmaSettings.append('                        Label')
    ...
    # sigmaSettings.append('                        Label')
    # sigmaSettings.append('                        {')
    # sigmaSettings.append('                            text: catalog.i18nc("@label", "Y max")')
    # sigmaSettings.append('                        }')
    # sigmaSettings.append('                        TextField')
    # sigmaSettings.append('                        {')
    # sigmaSettings.append('                            id: printheadYMaxField')
    # sigmaSettings.append('                            text: getHeadPolygonCoord("y", "max")')
    # sigmaSettings.append('                            validator: RegExpValidator { regExp: /[0-9\.]{0,6}/ }')
    # sigmaSettings.append('                            onEditingFinished: setHeadPolygon()')
    # sigmaSettings.append('                        }')
    # sigmaSettings.append('                        Label')
    ...
    # sigmaSettings.append('                        }')
    # sigmaSettings.append('')
    # sigmaSettings.append('                        Item { width: UM.Theme.getSize("default_margin").width; height: UM.Theme.getSize("default_margin").height }')
    # sigmaSettings.append('                        Item { width: UM.Theme.getSize("default_margin").width; height: UM.Theme.getSize("default_margin").height }')
    # sigmaSettings.append('                        Item { width: UM.Theme.getSize("default_margin").width; height: UM.Theme.getSize("default_margin").height }')
    # sigmaSettings.append('')
    # sigmaSettings.append('                        Label')
    # sigmaSettings.append('                        {')
    # sigmaSettings.append('                            text: catalog.i18nc("@label", "Gantry height")')
    # sigmaSettings.append('                        }')
    # sigmaSettings.append('                        TextField')
    # sigmaSettings.append('                        {')
    # sigmaSettings.append('                            id: gantryHeightField')
    # sigmaSettings.append('                            text: gantryHeightProvider.properties.value')
    # sigmaSettings.append('                            validator: RegExpValidator { regExp: /[0-9\.]{0,6}/ }')
    # sigmaSettings.append('                            onEditingFinished: { gantryHeightProvider.setPropertyValue("value", text) }')
    # sigmaSettings.append('                        }')
    # sigmaSettings.append('                        Label')
    # sigmaSettings.append('                        {')
    # sigmaSettings.append('                            text: catalog.i18nc("@label", "mm")')
    ...
    # sigmaSettings.append('            Row')
    # sigmaSettings.append('            {')
    # sigmaSettings.append('                spacing: UM.Theme.getSize("default_margin").width')
    # sigmaSettings.append('                anchors.left: parent.left')
    # sigmaSettings.append('                anchors.right: parent.right')
    # sigmaSettings.append('                height: parent.height - y')
    # sigmaSettings.append('                Column')
    # sigmaSettings.append('                {')
    # sigmaSettings.append('                    height: parent.height')
    # sigmaSettings.append('                    width: parent.width / 2')
    # sigmaSettings.append('                    Label')
    # sigmaSettings.append('                    {')
    # sigmaSettings.append('                        text: catalog.i18nc("@label", "Start Gcode")')
    # sigmaSettings.append('                    }')
    # sigmaSettings.append('                    TextArea')
    # sigmaSettings.append('                    {')
    # sigmaSettings.append('                        id: machineStartGcodeField')
    # sigmaSettings.append('                        width: parent.width')
    # sigmaSettings.append('                        height: parent.height - y')
    # sigmaSettings.append('                        font: UM.Theme.getFont("fixed")')
    # sigmaSettings.append('                        wrapMode: TextEdit.NoWrap')
    # sigmaSettings.append('                        text: machineStartGcodeProvider.properties.value')
    # sigmaSettings.append('                        onActiveFocusChanged:')
    # sigmaSettings.append('                        {')
    # sigmaSettings.append('                            if(!activeFocus)')
    # sigmaSettings.append('                            {')
    # sigmaSettings.append('                                machineStartGcodeProvider.setPropertyValue("value", machineStartGcodeField.text)')
    # sigmaSettings.append('                            }')
    # sigmaSettings.append('                        }')
    # sigmaSettings.append('                    }')
    # sigmaSettings.append('                }')
    ...
    # sigmaSettings.append('                    Label')
    # sigmaSettings.append('                    {')
    # sigmaSettings.append('                        text: catalog.i18nc("@label", "End Gcode")')
    # sigmaSettings.append('                    }')
    # sigmaSettings.append('                    TextArea')
    # sigmaSettings.append('                    {')
    # sigmaSettings.append('                        id: machineEndGcodeField')
    # sigmaSettings.append('                        width: parent.width')
    # sigmaSettings.append('                        height: parent.height - y')
    # sigmaSettings.append('                        font: UM.Theme.getFont("fixed")')
    # sigmaSettings.append('                        wrapMode: TextEdit.NoWrap')
    # sigmaSettings.append('                        text: machineEndGcodeProvider.properties.value')
    # sigmaSettings.append('                        onActiveFocusChanged:')
    # sigmaSettings.append('                        {')
    # sigmaSettings.append('                            if(!activeFocus)')
    # sigmaSettings.append('                            {')
    # sigmaSettings.append('                                machineEndGcodeProvider.setPropertyValue("value", machineEndGcodeField.text)')
    # sigmaSettings.append('                            }')
    # sigmaSettings.append('                        }')
    # sigmaSettings.append('                    }')
    # sigmaSettings.append('                }')
    ...
    # sigmaSettings.append('    }')
    # sigmaSettings.append('')
    # sigmaSettings.append('    function getHeadPolygonCoord(axis, minMax)')
    # sigmaSettings.append('    {')
    # sigmaSettings.append('        var polygon = JSON.parse(machineHeadPolygonProvider.properties.value);')
    # sigmaSettings.append('        var item = (axis == "x") ? 0 : 1')
    # sigmaSettings.append('        var result = polygon[0][item];')
    # sigmaSettings.append('        for(var i = 1; i < polygon.length; i++) {')
    # sigmaSettings.append('            if (minMax == "min") {')
    # sigmaSettings.append('                result = Math.min(result, polygon[i][item]);')
    # sigmaSettings.append('            } else {')
    # sigmaSettings.append('                result = Math.max(result, polygon[i][item]);')
    # sigmaSettings.append('            }')
    # sigmaSettings.append('        }')
    # sigmaSettings.append('        return Math.abs(result);')
    # sigmaSettings.append('    }')
    # sigmaSettings.append('')
    # sigmaSettings.append('    function setHeadPolygon()')
    # sigmaSettings.append('    {')
    # sigmaSettings.append('        var polygon = [];')
    # sigmaSettings.append('        polygon.push([-parseFloat(printheadXMinField.text), parseFloat(printheadYMaxField.text)]);')
    # sigmaSettings.append('        polygon.push([-parseFloat(printheadXMinField.text),-parseFloat(printheadYMinField.text)]);')
    # sigmaSettings.append('        polygon.push([ parseFloat(printheadXMaxField.text), parseFloat(printheadYMaxField.text)]);')
    # sigmaSettings.append('        polygon.push([ parseFloat(printheadXMaxField.text),-parseFloat(printheadYMinField.text)]);')
    # sigmaSettings.append('        machineHeadPolygonProvider.setPropertyValue("value", JSON.stringify(polygon));')
    # sigmaSettings.append('        manager.forceUpdate();')
    # sigmaSettings.append('    }')
    # sigmaSettings.append('')
    # sigmaSettings.append('    UM.SettingPropertyProvider')
    # sigmaSettings.append('    {')
    # sigmaSettings.append('        id: machineWidthProvider')
    # sigmaSettings.append('')
    # sigmaSettings.append('        containerStackId: Cura.MachineManager.activeMachineId')
    # sigmaSettings.append('        key: "machine_width"')
    # sigmaSettings.append('        watchedProperties: [ "value" ]')
    # sigmaSettings.append('        storeIndex: manager.containerIndex')
    # sigmaSettings.append('    }')
    # sigmaSettings.append('')
    # sigmaSettings.append('    UM.SettingPropertyProvider')
    # sigmaSettings.append('    {')
    # sigmaSettings.append('        id: machineDepthProvider')
    # sigmaSettings.append('')
    # sigmaSettings.append('        containerStackId: Cura.MachineManager.activeMachineId')
    # sigmaSettings.append('        key: "machine_depth"')
    # sigmaSettings.append('        watchedProperties: [ "value" ]')
    # sigmaSettings.append('        storeIndex: manager.containerIndex')
    # sigmaSettings.append('    }')
    # sigmaSettings.append('')
    # sigmaSettings.append('    UM.SettingPropertyProvider')
    # sigmaSettings.append('    {')
    # sigmaSettings.append('        id: machineHeightProvider')
    # sigmaSettings.append('')
    # sigmaSettings.append('        containerStackId: Cura.MachineManager.activeMachineId')
    # sigmaSettings.append('        key: "machine_height"')
    # sigmaSettings.append('        watchedProperties: [ "value" ]')
    # sigmaSettings.append('        storeIndex: manager.containerIndex')
    # sigmaSettings.append('    }')
    # sigmaSettings.append('')
    # sigmaSettings.append('    UM.SettingPropertyProvider')
    # sigmaSettings.append('    {')
    # sigmaSettings.append('        id: machinePrintOneAtATimeProvider')
    # sigmaSettings.append('')
    # sigmaSettings.append('        containerStackId: Cura.MachineManager.activeMachineId')
    # sigmaSettings.append('        key: "print_sequence"')
    # sigmaSettings.append('        watchedProperties: [ "enabled" ]')
    # sigmaSettings.append('        storeIndex: manager.containerIndex')
    # sigmaSettings.append('    }')
    # sigmaSettings.append('')
    # sigmaSettings.append('    UM.SettingPropertyProvider')
    # sigmaSettings.append('    {')
    # sigmaSettings.append('        id: machineAutoTemperatureProvider')
    # sigmaSettings.append('')
    # sigmaSettings.append('        containerStackId: Cura.MachineManager.activeMachineId')
    # sigmaSettings.append('        key: "material_flow_dependent_temperature"')
    # sigmaSettings.append('        watchedProperties: [ "enabled" ]')
    # sigmaSettings.append('        storeIndex: manager.containerIndex')
    # sigmaSettings.append('    }')
    # sigmaSettings.append('')
    # sigmaSettings.append('    UM.SettingPropertyProvider')
    # sigmaSettings.append('    {')
    # sigmaSettings.append('        id: gantryHeightProvider')
    # sigmaSettings.append('')
    # sigmaSettings.append('        containerStackId: Cura.MachineManager.activeMachineId')
    # sigmaSettings.append('        key: "gantry_height"')
    # sigmaSettings.append('        watchedProperties: [ "value" ]')
    # sigmaSettings.append('        storeIndex: manager.containerIndex')
    # sigmaSettings.append('    }')
    # sigmaSettings.append('')
    # sigmaSettings.append('    UM.SettingPropertyProvider')
    # sigmaSettings.append('    {')
    # sigmaSettings.append('        id: machineHeadPolygonProvider')
    # sigmaSettings.append('')
    # sigmaSettings.append('        containerStackId: Cura.MachineManager.activeMachineId')
    # sigmaSettings.append('        key: "machine_head_with_fans_polygon"')
    # sigmaSettings.append('        watchedProperties: [ "value" ]')
    # sigmaSettings.append('        storeIndex: manager.containerIndex')
    # sigmaSettings.append('    }')
    # sigmaSettings.append('')
    ...
    # sigmaSettings.append('    UM.SettingPropertyProvider')
    # sigmaSettings.append('    {')
    # sigmaSettings.append('        id: machineStartGcodeProvider')
    # sigmaSettings.append('')
    # sigmaSettings.append('        containerStackId: Cura.MachineManager.activeMachineId')
    # sigmaSettings.append('        key: "machine_start_gcode"')
    # sigmaSettings.append('        watchedProperties: [ "value" ]')
    # sigmaSettings.append('        storeIndex: manager.containerIndex')
    # sigmaSettings.append('    }')
    # sigmaSettings.append('')
    # sigmaSettings.append('    UM.SettingPropertyProvider')
    # sigmaSettings.append('    {')
    # sigmaSettings.append('        id: machineEndGcodeProvider')
    # sigmaSettings.append('')
    # sigmaSettings.append('        containerStackId: Cura.MachineManager.activeMachineId')
    # sigmaSettings.append('        key: "machine_end_gcode"')
    # sigmaSettings.append('        watchedProperties: [ "value" ]')
    # sigmaSettings.append('        storeIndex: manager.containerIndex')
    # sigmaSettings.append('    }')
    # sigmaSettings.append('')
