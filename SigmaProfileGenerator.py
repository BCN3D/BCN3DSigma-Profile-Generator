...
def createCura2Files():
    ...
    with open('Cura 2/plugins/PostProcessingPlugin/scripts/'+cura2PostProcessingPluginName.replace(' ','')+'.py', 'w') as f:
        ...
        lines.append(r'                            if line.startswith(idleExtruder) and lines[tempIndex+1].startswith("G92 E0") and lines[tempIndex+2].startswith("G1 E") and lines[tempIndex+3].startswith("G92 E0"):'+'\n')
        lines.append(r'                                layer = layer.replace(lines[tempIndex]+"\n"+lines[tempIndex+1]+"\n"+lines[tempIndex+2]+"\n"+lines[tempIndex+3]+"\n", "")'+'\n')
        ...
        lines.append(r'                for tempIndex in range(len(lines)):'+'\n')
        lines.append(r'                    line = lines[tempIndex]'+'\n')
        lines.append(r'                    if (line == "T0" or line == "T1") and lines[tempIndex+1] == "G92 E0" and lines[tempIndex+2] == "G91" and "G1 F" in lines[tempIndex+3] and lines[tempIndex+4] == "G90":'+'\n')
        lines.append(r'                        layer = layer.replace(lines[tempIndex+3], lines[tempIndex+3].split("Z")[0]+"Z"+str(zHopDistance))'+'\n')
        ...
        lines.append(r'                for tempIndex in range(len(lines)):'+'\n')
        lines.append(r'                    line = lines[tempIndex]'+'\n')
        lines.append(r'                    if line == "T0" and lines[tempIndex+1] == "G92 E0" and lines[tempIndex+2] == "G91" and "G1 F" in lines[tempIndex+3] and lines[tempIndex+4] == "G90":'+'\n')
        lines.append(r'                        lineCount = 6 # According to extruder_start_gcode in Sigma Extruders definitions'+'\n')
        lines.append(r'                        while not lines[tempIndex+lineCount].startswith(";TYPE"):'+'\n')
        lines.append(r'                            lineCount += 1'+'\n')
        ...
        lines.append(r'                        break'+'\n')
        lines.append(r''+'\n')
        lines.append(r'                    if line == "T1" and lines[tempIndex+1] == "G92 E0" and lines[tempIndex+2] == "G91" and "G1 F" in lines[tempIndex+3] and lines[tempIndex+4] == "G90":'+'\n')
        lines.append(r'                        lineCount = 6 # According to extruder_start_gcode in Sigma Extruders definitions'+'\n')
        lines.append(r'                        while not lines[tempIndex+lineCount].startswith(";TYPE"):'+'\n')
        lines.append(r'                            lineCount += 1'+'\n')
        lines.append(r'                        primeLine = lines[tempIndex+lineCount+1]'+'\n')
        ...
        lines.append(r"    curveGrowth = 1 # Here we assume the growth curve is constant for all materials. Change this value if it's not"+'\n')
    ...
    with open('Cura 2/plugins/'+machineSettingsPluginName+'/__init__.py', 'w') as f:
        lines = []
        lines.append(r'from . import SigmaSettingsAction'+'\n')
        lines.append(r''+'\n')
        ...
        lines.append(r'    return { "machine_action": SigmaSettingsAction.SigmaSettingsAction() }'+'\n')
        f.writelines(lines)
    with open('Cura 2/plugins/'+machineSettingsPluginName+'/'+machineSettingsPluginName+'.py', 'w') as f:
        ...
        lines.append(r'class SigmaSettingsAction(MachineAction):'+'\n')
        lines.append(r'    def __init__(self, parent = None):'+'\n')
        lines.append(r'        super().__init__("SigmaSettingsAction", catalog.i18nc("@action", "Sigma Settings"))'+'\n')
        lines.append(r'        self._qml_url = "SigmaSettingsAction.qml"'+'\n')
        lines.append(r''+'\n')
        ...
        lines.append(r'                Logger.log("d", "Not attaching SigmaSettingsAction to %s; Multi-extrusion printers are not supported", container.getId())'+'\n')
        lines.append(r'                return'+'\n')
        lines.append(r''+'\n')
    ...
    with open('Cura 2/plugins/'+machineSettingsPluginName+'/'+machineSettingsPluginName+'.qml', 'w') as f:
        ...
        lines.append(r'            width: parent.width'+'\n')
        lines.append(r'            wrapMode: Text.WordWrap'+'\n')
        lines.append(r'            visible: Cura.MachineManager.activeMachineId == "Sigma"'+'\n')
        lines.append(r'            text: catalog.i18nc("@label", "Attention! In order to get the best results with your BCN3D Sigma, please make sure the Sigma Vitamins plugin is enabled under Extensions > Post Processing > Modify G-Code.")'+'\n')
        lines.append(r'        }'+'\n')
        lines.append(r''+'\n')
